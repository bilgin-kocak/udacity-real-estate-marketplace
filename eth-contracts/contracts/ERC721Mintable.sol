...
contract Ownable {
    ...
    //  4) fill out the transferOwnership function: transfer control of the contract to a newOwner.
    function transferOwnership(address newOwner) public onlyOwner {
        require(newOwner != address(0), "Invalid Address");    // make sure the new owner is a real address
...
contract ERC721 is Pausable, ERC165 {
    ...
    function approve(address to, uint256 tokenId) public {
        ...
        // check the given address to not be the owner of the tokenId
        require(to != _tokenOwner[tokenId], "Token is already owned by receipient");
        // check the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(isApprovedForAll(_owner, msg.sender) || msg.sender == _owner, "Not allowed to approve");
...
/**
* @dev Create CustomERC721Token contract that inherits from the ERC721Metadata contract.
* Pass in appropriate values for the inherited ERC721Metadata contract
* make the base token uri: https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/
*/
contract R2Token is ERC721Metadata("R2 Token", "R2", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/"){
    // public mint() which can only be executed by the contract owner
    // and take in a 'to' address, tokenId, and tokenURI as parameters
    // and return a true boolean upon completion of the function
